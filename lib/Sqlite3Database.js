...
var fs = require('fs');
var child = require("child_process").fork(__dirname + '/ChildQuery');
...
var MysqlDatabase = module.exports = Database.extend({
  ...
  connect: function connect(callback) {
    ...
    // now start the child connection
    child.send({
      msg: "newConnect",
      config: config
    });
  ...
  query: function query(sql, callback) {
    // console.log(this.connection.each.toString());
    child.on("message", function (data) {
      if (data && data.sql == sql) {
