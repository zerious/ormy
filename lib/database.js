...
var Database = module.exports = Flagger.extend({
   ...
   */
  query: function (sql, passedPlan) {
    var self = this;
    var plan = self.makePlan(passedPlan);
    plan.sql = sql;
    if (!passedPlan || plan.stream) {
      var stream = self.connection.query(sql);
  ...
  /**
   * Decorate or create a plan.
   */
  makePlan: function (plan) {
    var self = this;
    if (plan) {
      plan.base = self.plan;
    }
   ...
   */
  find: function (model, options, passedPlan) {
    var self = this;
  ...
  create: function (model, item, plan) {
    ...
    plan = plans.before(plan, 'ok', function (result) {
      self.decorateItem(model, item);
      item.id = result.insertId;
      if (self.auditModel && (model != self.auditModel)) {
        self.audit('INSERT', model, item, plan);
      }
    });
  ...
  update: function (model, item, plan) {
    ...
    plan = plans.before(plan, 'ok', function (item) {
      ...
      item.id = id;
      if (self.auditModel) {
        self.audit('UPDATE', model, item, plan);
      }
    });
      ...
      self.query('SELECT *' + from, function (e, items) {
        if (items) {
          items.forEach(function (item) {
            self.audit('DELETE', model, item, plan);
          });
   ...
   */
  audit: function (operation, model, item, plan) {
    var self = this;

    plan = plan || 0;
    var response = plan.response || 0;
    var request = response.request || 0;
    var session = request.session || 0;
    var userId = session.userId || plan.userId || 0;
